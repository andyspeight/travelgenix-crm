...
- [x] **2. Real email sending** — the first live channel (Brevo, same provider as Luna Marketing):
  ...
  - **Provider**: `lib/email/brevo.ts` — plain REST (no SDK), 10s timeout, fails closed. Configured by exactly two env vars (`BREVO_API_KEY`, `EMAIL_FROM`; optional `EMAIL_FROM_NAME`). Unset → `emailConfigured()` is false and every send button falls back to the old mailto behaviour. Nothing breaks, nothing pretends to send.
  - **API**: `POST /api/email/send` — rate-limited 30/min, resolves the recipient (contact or raw address), reads current consent from the ledger (legacy-flag fallback), checks suppressions, then policy-gates, dispatches, and records: an `email_sends` audit row (kept even for failures), an `email_out` interaction on the customer timeline, `email.sent` on the event spine. `POST /api/email/webhook` (secret-token auth, let through the access gate) takes Brevo bounce/complaint events → suppression row + audit row flip + `email.bounced`.
  - **Schema**: `supabase/migrations/20260729090000_email_channel.sql` — `email_sends` (full audit: to/subject/body/purpose/context/status/provider id) + `email_suppressions` (unique per agency+address with reason).
  - **UI**: when the provider is configured — Inbox "Send as is / Send edited" genuinely sends and confirms "Sent to … and recorded on the timeline"; Enquiries "Respond now" opens an inline composer, the clock stops only after a real send; a journey draft's button becomes "Send now" (Luna drafted, a human dispatched). Settings shows the Brevo row so it's obvious which mode the workspace is in. Bulk segment email stays mailto for now — deliberate; bulk through the provider needs sequences + per-send caps (next round).
  - **ACTION FOR ANDY**: in Vercel add `BREVO_API_KEY` + `EMAIL_FROM` (a Brevo-verified sender) (+ optional `EMAIL_FROM_NAME`). For bounce handling: add `EMAIL_WEBHOOK_SECRET` and point a Brevo transactional webhook at `https://<app>/api/email/webhook?token=<secret>` for hard bounce / blocked / invalid / spam events.
